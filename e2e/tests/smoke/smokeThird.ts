    ...
    await testLogger(test, httpLogger, async () => {
        ...
        await testStep('On Energy Australia website, verify First Menu Item', async () => {
            ...
            await it(`should have first menu name as Electricity and gas`, async() => {
                await test.expect(menuItem.nth(0).innerText).eql('Electricity and gas', logFailureMessage('First Menu Name was not Electricity and gas'));
            });
            await it(`should contain "Compare electricity and gas" in its's sub menu`, async () => {
                await test.expect(subMenu.nth(0).innerText).contains('Compare electricity and gas', logFailureMessage('Sub Menu does not contain Compare electricity and gas'));
            })
        });
        ...
        await testStep('On Energy Australia website, verify Second Menu Item', async () => {
            ...
            await it(`should have second menu name as Moving house`, async() => {
                await test.expect(menuItem.nth(1).innerText).eql('Moving house', logFailureMessage('Second Menu Name was not Moving house'));
            });
            await it(`should contain "Make it a good move" in its's sub menu`, async () => {
                await test.expect(subMenu.nth(1).innerText).contains('Make it a good move', logFailureMessage('Sub Menu does not contain Make it a good move'));
            })
        });
        ...
        await testStep('On Energy Australia website, verify Third Menu Item', async () => {
            ...
            await it(`should have third menu name as Bills and accounts`, async() => {
                await test.expect(menuItem.nth(2).innerText).eql('Bills and accounts', logFailureMessage('Third Menu Name was not Bills and accounts'));
            });
            await it(`should contain "Manage your account" in its's sub menu`, async () => {
                await test.expect(subMenu.nth(2).innerText).contains('Manage your account', logFailureMessage('Sub Menu does not contain Manage your account'));
            })
        });
    });
});
